...
class Strategy():
    ...
    def initPlot(self) -> (plt, plt):
        ...
        self.ax.plot(self.support_x_list, self.support_y_list, color="violet", linestyle="dashed", linewidth=0.5)
        self.ax.plot(self.press_x_list, self.press_y_list, color="purple", linestyle="dashed", linewidth=0.5)
    ...
    def Signal(self, n: int, direction: str):
        ...
        #     support_x_ls = list(filter(lambda x: x <= n and x > n - 60, self.support_x_list))
        #     support_y_ls = [self.y[k] for k in support_x_ls]
        #     press_x_ls = list(filter(lambda x: x <= n and x > n - 60, self.press_x_list))
        #     press_y_ls = [self.y[k] for k in press_x_ls]
        #     if len(support_y_ls) < 2 or len(press_y_ls) < 2:
        #         if self.y[n] > self.y[n - 59]:
        #             sig_type, diff = "TRE1", 0
        #             check_buy_signal = False
        #             self.ax.plot(support_x_ls, support_y_ls, color="darkorange")
        #             self.ax.plot(press_x_ls, press_y_ls, color="olive")
        #     elif (np.diff(support_y_ls) >= 0).all() and (np.diff(press_y_ls) >= 0).all() \
